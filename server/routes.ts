...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/templates/ai-cv", isAuthenticated, hasActiveSubscription, async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // 1. Check daily usage limits (10 templates per day for premium users)
      ...
      .where(and(
        ...
        gte(generatedTemplates.createdAt, todayStart)
      ));
      
      const dailyCount = recentTemplates[0]?.count || 0;
      
      if (dailyCount >= 10) {
        return res.status(429).json({
          error: "Daily template generation limit reached",
          message: "You've reached your daily limit of 10 template generations. Please try again tomorrow."
        });
      }
      
      // 2. Generate a secure watermark ID
      const deviceInfo = req.headers['user-agent'] || 'unknown';
      const clientIp = req.ip || req.socket.remoteAddress || 'unknown';
      const timestamp = Date.now();
      const watermarkData = `${userId}-${timestamp}-${clientIp}`;
      const watermarkId = createHash('sha256').update(watermarkData).digest('hex').substring(0, 16);
      ...
      // 4. Add visible and invisible watermarking
      const watermarkedTemplate = {
        ...template,
        content: template.content + `\n\n[Generated by ATSBoost for ${userProfile.name} - ID: ${watermarkId}]`,
        watermarkId
      };
      
      // 5. Store template usage information
      ...
      // 6. Update user's template usage count in subscription
      await db.update(subscriptions)
        .set({ 
          templatesUsed: sql`templatesUsed + 1`,
          updatedAt: new Date()
        })
      ...
      res.json({
        ...
        remainingToday: 10 - dailyCount - 1,
        watermarkId
      });
    } catch (error: any) {
