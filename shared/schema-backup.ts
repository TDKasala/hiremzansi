...
export const notifications = pgTable("notifications", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(),
  type: text("type").notNull(), // match_found, payment_received, profile_viewed, job_posted, etc.
  title: text("title").notNull(),
  ...
  relatedEntityId: integer("related_entity_id"), // ID of related job, match, payment, etc.
  relatedEntityType: text("related_entity_type"), // job, match, payment, etc.
  metadata: json("metadata"), // Additional data about the notification
  expiresAt: timestamp("expires_at"), // When notification should be automatically archived
  deliveryMethod: text("delivery_method").array().default(['in_app']), // in_app, email, sms, whatsapp
...
export const paymentTransactions = pgTable("payment_transactions", {
  ...
  currency: text("currency").default("ZAR").notNull(),
  paymentType: text("payment_type").notNull(), // job_seeker_match, recruiter_access, premium_features
  userType: text("user_type").notNull(), // job_seeker, recruiter
  status: text("status").default("pending").notNull(), // pending, processing, completed, failed, refunded, cancelled
  paymentProvider: text("payment_provider").default("payfast").notNull(),
  transactionId: text("transaction_id").unique(),
  providerTransactionId: text("provider_transaction_id"),
  relatedEntityId: integer("related_entity_id"), // ID of job match, job posting, etc.
  relatedEntityType: text("related_entity_type"), // job_match, job_posting, premium_feature
  paymentUrl: text("payment_url"), // Payment gateway URL
  returnUrl: text("return_url"), // Success return URL
  cancelUrl: text("cancel_url"), // Cancel return URL
  notifyUrl: text("notify_url"), // Webhook notification URL
  description: text("description"),
...
// Premium job matches with payment tracking
export const premiumJobMatches = pgTable("premium_job_matches", {
  id: serial("id").primaryKey(),
  jobSeekerUserId: integer("job_seeker_user_id").references(() => users.id).notNull(),
  recruiterUserId: integer("recruiter_user_id").references(() => users.id).notNull(),
  ...
  matchData: json("match_data"), // Detailed AI matching analysis
...
export const premiumJobMatchesRelations = relations(premiumJobMatches, ({ one }) => ({
  jobSeeker: one(users, {
    fields: [premiumJobMatches.jobSeekerUserId],
    references: [users.id],
  }),
  recruiter: one(users, {
    fields: [premiumJobMatches.recruiterUserId],
    references: [users.id],
  }),
  jobPosting: one(jobPostings, {
...
// Removed duplicate notification table - using the enhanced version above
