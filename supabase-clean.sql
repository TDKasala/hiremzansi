-- Hire Mzansi Clean Database Setup
-- Fixed ON CONFLICT and constraint issues
...
-- Insert default plans (fixed version)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM plans WHERE name = 'Free Trial') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM plans WHERE name = 'Deep Analysis') THEN
        INSERT INTO plans (name, description, price, billing_cycle, features, cv_analysis_credits) VALUES
    ...
    IF NOT EXISTS (SELECT 1 FROM plans WHERE name = 'Monthly Premium') THEN
        INSERT INTO plans (name, description, price, billing_cycle, features, cv_analysis_credits) VALUES
    ...
    IF NOT EXISTS (SELECT 1 FROM plans WHERE name = 'Yearly Premium') THEN
        INSERT INTO plans (name, description, price, billing_cycle, features, cv_analysis_credits) VALUES
...
-- Insert default skills (fixed version)
DO $$
BEGIN
