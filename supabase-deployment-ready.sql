-- Hire Mzansi Production-Ready Database Setup
-- Complete schema with flexible authentication options
...
-- Users table with flexible auth support
CREATE TABLE IF NOT EXISTS users (
    ...
    supabase_user_id UUID UNIQUE, -- Link to Supabase auth if needed
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW()
);

-- Rest of schema (optimized for production)
...
INSERT INTO plans (name, description, price, billing_cycle, features, cv_analysis_credits, sort_order) VALUES
('Free Trial', '3 free CV analyses valid for 3 days', 0.00, 'trial', 
 '{"job_matching": "free", "basic_ats": true, "duration_days": 3, "detailed_report": false}', 3, 1),
('Deep Analysis', 'One-time comprehensive CV analysis', 25.00, 'one_time', 
 '{"job_matching": "free", "detailed_report": true, "bbbee_optimization": true, "lifetime_access": true}', 1, 2),
('Monthly Premium', '50 CV analyses per month', 100.00, 'monthly', 
 '{"job_matching": "free", "detailed_report": true, "real_time_editor": true, "priority_support": true}', 50, 3),
('Yearly Premium', '50 analyses per month with 10% discount', 1080.00, 'yearly', 
 '{"job_matching": "free", "detailed_report": true, "real_time_editor": true, "priority_support": true, "discount": "10%"}', 50, 4)
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sa_profiles_user_id ON sa_profiles(user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cvs_user_id ON cvs(user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cvs_active ON cvs(user_id, is_default);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ats_scores_cv_id ON ats_scores(cv_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ats_scores_user_score ON ats_scores(user_id, overall_score);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_subscriptions_user_status ON subscriptions(user_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_postings_active ON job_postings(is_active, created_at) WHERE is_active = true;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_postings_employer ON job_postings(employer_id, is_active);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_postings_location ON job_postings(location) WHERE is_active = true;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_matches_seeker ON job_matches(job_seeker_id, created_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_matches_posting ON job_matches(job_posting_id, match_score);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_matches_score ON job_matches(match_score) WHERE match_score >= 70;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_employers_user_id ON employers(user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_employers_verified ON employers(is_verified) WHERE is_verified = true;
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_transactions_user ON payment_transactions(user_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_skills_user_id ON user_skills(user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_whatsapp_settings_user_id ON whatsapp_settings(user_id);

-- =========================================
-- OPTION 1: DISABLE RLS FOR APPLICATION-LEVEL SECURITY
-- =========================================

-- For production with external authentication, disable RLS
-- and handle security in your application layer
...
-- Function to get user's available CV credits
...
-- Function for basic job matching (free for all users)
...
DECLARE
    ...
    total_job_words INTEGER;
BEGIN
    -- Simple keyword matching
    cv_words := regexp_split_to_array(lower(cv_text), '\W+');
    job_words := regexp_split_to_array(
        lower(job_title || ' ' || job_description || ' ' || COALESCE(job_requirements, '')), 
        '\W+'
    );
    
    -- Filter meaningful words (length > 2)
    cv_words := array_remove(cv_words, '');
    job_words := array_remove(job_words, '');
    ...
    total_job_words := array_length(
        array(SELECT DISTINCT unnest(job_words) WHERE length(unnest(job_words)) > 2), 1
    );
    
    -- Calculate percentage match
    IF total_job_words > 0 THEN
        score := LEAST(100, (common_count * 100 / total_job_words));
    END IF;
